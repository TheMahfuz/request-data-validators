...
module.exports = class RequestData {
    ...
    type(type, custom_message = '') {
        ...
        if (this.value) {
            if (type == 'integer') {
                if (!utils.isInteger(this.value)) err = 1;
                else this.value = parseInt(this.value)

            } else if (type == 'number') {
                if (!utils.isNumber(this.value)) err = 1;

            } else if (type == 'email') {
                if (!utils.isEmail(this.value)) err = 1;
                ...
                if (!this.isDate(this.value)) err = 1;
    ...
    validate(redirect = true) {
        ...
        let resp = {
            result_code: 1,
            time: moment().format('YYYY-MM-DD H:m:ss'),
            error: {}
        };
        if (vs.error > 0) {
